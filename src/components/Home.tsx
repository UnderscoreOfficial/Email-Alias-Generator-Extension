import { ScrollArea, Tabs, useMantineColorScheme } from "@mantine/core";
...
export default function Home() {
  ...
  const [disable_storing_aliases] = useStorage<DisableStoringAliases>(
  ...
  useEffect(() => {
    if (default_tab.current && active_tab.length < 1) {
      ...
      // this logic is depricated now but needs to be carefully looked into before removing something about this is breaking create
      // e.g when adding a create sometimes it would be extreamly laggy or just not add or take multiple seconds to add very odd behavior
      // only possible guess is something with the useStorage import of default_tab? for now I will leve this since really its not doing harm
      //
  ...
  return (
    <Tabs
      ...
      <Tabs.List
        className={`fixed top-0 left-0 right-0 z-50 ${colorScheme == "light" ? "bg-white" : "bg-gray-primary"}`}>
        <Tabs.Tab
          className="w-1/3 tab-hover"
          value="settings"
          leftSection={
            <svg
              className="purple-custom"
              color="purple"
        ...
        <Tabs.Tab
          className={`w-1/3 ${disable_storing_aliases ? "" : "tab-hover"}`}
          value="aliases"
      ...
      <Tabs.Panel value="settings" className="flex justify-center items-center">
        {is_mobile ? (
          ...
          <ScrollArea
            type="scroll"
            scrollHideDelay={750}
            className="h-[536px] w-full top-12 absolute">
            <Settings />
          </ScrollArea>
        )}
      </Tabs.Panel>
