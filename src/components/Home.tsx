...
export default function Home() {
  ...
  const [active_tab, setActiveTab] = useState<string>(default_tab);
  const { colorScheme } = useMantineColorScheme();
  const [disable_storing_aliases] = useStorage("disable_storing_aliases", false);

  return (
    <Tabs value={active_tab || default_tab || "aliases"} onChange={(value) => setActiveTab(value || "")} className="relative">
